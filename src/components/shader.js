...
const ShaderArtComponent = () => {
  useEffect(() => {
    const loadShaderArt = async () => {
      try {
        const { ShaderArt } = await import('https://cdn.skypack.dev/shader-art');
        const { UniformPlugin } = await import('https://cdn.skypack.dev/@shader-art/plugin-uniform');
        ShaderArt.register([() => new UniformPlugin()]);
      } catch (error) {
        console.error("Failed to load ShaderArt or UniformPlugin", error);
      }
    };
  ...
  return (
    <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, opacity: 0.5 }}>
      <shader-art autoplay>
